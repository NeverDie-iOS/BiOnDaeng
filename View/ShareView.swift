...
struct ShareView: View {
    ...
    func shareToInstagram() {
        ...
        // 인스타그램에서 요구하는 이미지 데이터로 변환
        guard let imageData = imageToShare.pngData() else {
        ...
        let instagramImageURL = temporaryDirectory.appendingPathComponent("instagram.png")
        ...
        // UIActivityViewController를 사용하여 인스타그램에 공유
        let activityViewController = UIActivityViewController(activityItems: [instagramImageURL], applicationActivities: nil)
        activityViewController.excludedActivityTypes = [.postToFacebook, .postToTwitter] // 필요시 제외할 액티비티 타입 설정

        // 현재의 ViewController에서 Activity View Controller 표시
        if let windowScene = UIApplication.shared.connectedScenes.first as? UIWindowScene,
           let rootViewController = windowScene.windows.first?.rootViewController {
            rootViewController.present(activityViewController, animated: true, completion: nil)
        }
